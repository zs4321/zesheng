...
# ── PUBG Mobile Style Colors ──
PUBG_BG=(15,15,20); PUBG_DARK=(25,25,35); PUBG_CARD=(35,35,50)
PUBG_ACCENT=(255,150,0); PUBG_GOLD=(255,215,0); PUBG_RED=(230,50,50)
PUBG_GREEN=(100,200,100); PUBG_BLUE=(60,140,220); PUBG_ORANGE=(255,140,0)
PUBG_KILL=(255,220,100); PUBG_WHITE=(240,240,240)
CHICKEN_YELLOW=(255,230,100); CHICKEN_GOLD=(255,200,50)
...
class Game:
    def __init__(self):
        ...
        # Menu background images (use relative path from script / PyInstaller bundle)
        self.menu_sky=None; self.menu_ground=None; self.menu_ruins=None
        try:
            if getattr(sys, 'frozen', False):
                base = os.path.join(sys._MEIPASS, "menu_assets")
            else:
                base = os.path.join(os.path.dirname(os.path.abspath(__file__)), "menu_assets")
        ...
        # Kill feed & coin animation
        self.kill_feed=[]  # list of (text, timer, color)
        self.coin_anim=0   # coin count-up animation timer
        self.coin_earned=0 # coins earned this round
        self.chicken_timer=0  # chicken dinner celebration timer
    ...
    def reset(self):
        ...
        self.kill_feed=[]; self.coin_anim=0; self.coin_earned=0
    ...
    def _draw_weapon_select(self):
        """PUBG Mobile style weapon select screen."""
        for y in range(H):
            r = y / H
            c = (int(PUBG_BG[0] * (1 - r * 0.3)),
                 int(PUBG_BG[1] * (1 - r * 0.25)),
                 int(PUBG_BG[2] * (1 - r * 0.15)))
            pygame.draw.line(self.screen, c, (0, y), (W, y))
        for x in range(0, W, 40):
            pygame.draw.line(self.screen, (20, 20, 28), (x, 0), (x, H))
        for y in range(0, H, 40):
            pygame.draw.line(self.screen, (20, 20, 28), (0, y), (W, y))

        pygame.draw.rect(self.screen, PUBG_ACCENT, (0, 0, W, 3))
        dtxt(self.screen, "选 择 武 器", (W // 2, 50), 36, PUBG_WHITE, True, True)
        pygame.draw.line(self.screen, PUBG_ACCENT, (W // 2 - 180, 82), (W // 2 + 180, 82), 2)
        dtxt(self.screen, f"金币: {self.coins}", (W - 100, 55), 14, CHICKEN_GOLD, ctr=True, shd=False)
        ...
        for i, wk in enumerate(self.warehouse):
            ...
            if selected:
                bg = (60, 70, 70) if hvr else (45, 55, 55)
                bdr = PUBG_GOLD
            else:
                bg = (40, 40, 50) if hvr else PUBG_DARK
                bdr = PUBG_GOLD if hvr else GRAY

            pygame.draw.rect(self.screen, (8, 8, 14), (rx + 2, ry + 2, rw, rh), border_radius=10)
            pygame.draw.rect(self.screen, bg, (rx, ry, rw, rh), border_radius=10)
            pygame.draw.rect(self.screen, bdr, (rx, ry, rw, rh), 2 if selected else 1, border_radius=10)

            if selected:
                pygame.draw.circle(self.screen, PUBG_GOLD, (rx + rw - 22, ry + 22), 11)
                dtxt(self.screen, "✓", (rx + rw - 22, ry + 22), 14, BLACK, True, True, False)

            dtxt(self.screen, w["name"], (rx + rw // 2, ry + 18), 20, PUBG_WHITE, True, True, False)
            dtxt(self.screen, f"伤害: {w['dmg']} | 射速: {w['rate']}", (rx + 12, ry + 45), 12, GRAY, shd=False)
            dtxt(self.screen, f"弹匣: {w['mag']} | 备弹: {self.ammo.get(wk, 0)}/{self.reserve.get(wk, 0)}",
                 (rx + 12, ry + 65), 12, CHICKEN_GOLD, shd=False)
            wc = WCOL[wk]
            pygame.draw.rect(self.screen, wc, (rx + 12, ry + 85, 18, 18), border_radius=3)
            mode_label = "自动" if w["auto"] else "半自动"
            dtxt(self.screen, mode_label, (rx + 35, ry + 88), 11, PUBG_GREEN if w["auto"] else GRAY, shd=False)

        # Start button
        btn_x, btn_y = W // 2 - 100, H - 95
        has_sel = len(self.ws_selected) > 0
        mx, my = pygame.mouse.get_pos()
        btn_hvr = btn_x < mx < btn_x + 200 and btn_y < my < btn_y + 50
        btn_bg = (70, 60, 25) if btn_hvr and has_sel else (50, 40, 20) if has_sel else (30, 30, 35)
        btn_bdr = PUBG_GOLD if btn_hvr and has_sel else GRAY if has_sel else (55, 55, 55)
        ...
        btn_col = PUBG_WHITE if has_sel else (80, 80, 80)
        dtxt(self.screen, "⚔ 开始战斗", (btn_x + 100, btn_y + 26), 22, btn_col, True, True, False)
        if not has_sel:
            dtxt(self.screen, "⚠ 请至少选择一把武器", (W // 2, btn_y - 12), 13, PUBG_RED, ctr=True)

        # Bottom bar
        for i in range(25):
            y = H - 25 + i; r = i / 25
            c = (int(PUBG_ACCENT[0] * (1 - r * 0.6)),
                 int(PUBG_ACCENT[1] * (1 - r * 0.6)),
                 int(PUBG_ACCENT[2] * (1 - r * 0.6)))
            pygame.draw.line(self.screen, c, (0, y), (W, y))
        for x in range(-25, W + 25, 50):
            pygame.draw.polygon(self.screen, (5, 5, 10),
                              [(x, H - 25), (x + 25, H - 25), (x + 40, H), (x + 15, H)])
        dtxt(self.screen, "ESC 返回 | 点击武器选择 | ENTER 开始",
             (W // 2, H - 15), 13, BLACK, True, True, False)
    ...
    def update(self):
        ...
        # Coin count-up animation (handled for all states)
        if self.coin_anim > 0:
            self.coin_anim -= 1
    ...
    def _update_play(self):
        ...
        for i in range(len(self.kill_feed)):
            text, timer, color = self.kill_feed[i]
            self.kill_feed[i] = (text, timer - 1, color)
        self.kill_feed = [kf for kf in self.kill_feed if kf[1] > 0]
        ...
        if self.cwave>=self.twaves and alive==0 and self.wdly<=0:
            ...
            if self.mode=="level":
                ...
                earned = diff*50 + self.kills*5 + self.score//10
                self.coin_earned = earned
                self.coins += earned
                if self.clvl>=self.max_ul:
                    self.max_ul=min(8,self.clvl+1)
            else:
                earned = 100 + self.kills*5 + self.score//10
                self.coin_earned = earned
                self.coins += earned
            self.coin_anim = 120  # Coin count-up animation
            self.kill_feed.append((f"💰 +{earned} 金币!", 120, CHICKEN_GOLD))
            write_save(self.user_name,self.max_ul,int(self.playtime),self.coins,self.warehouse)
        ...
        for _ in range(w["pellets"]):
            ...
            if hit_e:
                ...
                if hit_e["hp"]<=0:
                    ...
                    # Kill feed entry
                    enemy_name = hit_e["type"]
                    ename_map = {"basic":"步兵","fast":"突击兵","shooter":"狙击手","heavy":"重装兵","boss":"BOSS"}
                    ename = ename_map.get(enemy_name, enemy_name)
                    self.kill_feed.append((f"击杀 {ename} +{ETYPES[hit_e['type']]['sc']}分",
                                          60, PUBG_KILL))
        ...
        for e in self.enemies:
            ...
            if d<4:
                ...
                if e["hp"]<=0:
                    ...
                    ename_map = {"basic":"步兵","fast":"突击兵","shooter":"狙击手","heavy":"重装兵","boss":"BOSS"}
                    ename = ename_map.get(e["type"], e["type"])
                    self.kill_feed.append((f"💣 炸飞 {ename} +{ETYPES[e['type']]['sc']}分",
                                          60, PUBG_ORANGE))
    ...
    def _draw_login(self):
        """PUBG Mobile style login screen."""
        # Gradient dark background
        for y in range(H):
            r = y / H
            c = (int(PUBG_BG[0] * (1 - r * 0.3)),
                 int(PUBG_BG[1] * (1 - r * 0.3)),
                 int(PUBG_BG[2] * (1 - r * 0.2)))
            pygame.draw.line(self.screen, c, (0, y), (W, y))

        # Animated particles in background
        for p in self.cparts:
            a = p["a"]
            c = (int(255 * a * 0.3), int(180 * a * 0.2), int(80 * a * 0.15))
            pygame.draw.circle(self.screen, c, (int(p["x"]), int(p["y"])), int(p["sz"]))

        # Top decorative bar
        pygame.draw.rect(self.screen, PUBG_ACCENT, (0, 0, W, 3))
        for i in range(3):
            pygame.draw.rect(self.screen, PUBG_GOLD if i == 1 else PUBG_ACCENT,
                           (W // 2 - 60 + i * 60, 3, 55, 2))

        # Game title with glow effect
        pulse = math.sin(self.t * 0.03) * 0.15 + 0.85
        title_y = 100
        # Glow
        for layer in range(3):
            alpha = int(40 * (3 - layer) * pulse)
            glow_col = (min(255, PUBG_ACCENT[0] + layer * 20),
                        min(255, PUBG_ACCENT[1] - layer * 10),
                        min(255, PUBG_ACCENT[2]))
            dtxt(self.screen, "W A R F R O N T  3 D",
                 (W // 2 + 1, title_y + 1 - layer), 52 - layer * 2,
                 glow_col, True, True, False)

        dtxt(self.screen, "W A R F R O N T  3 D",
             (W // 2, title_y), 48, PUBG_ACCENT, True, True, False)

        # Subtitle with chicken dinner reference
        dtxt(self.screen, "— 大吉大利，今晚吃鸡 —",
             (W // 2, title_y + 52), 18, CHICKEN_GOLD, ctr=True)

        # Divider line with gradient effect
        div_y = title_y + 80
        for i in range(3):
            alpha = 255 - i * 80
            c = (PUBG_ACCENT[0], PUBG_ACCENT[1], PUBG_ACCENT[2])
            pygame.draw.line(self.screen, c, (W // 2 - 150 + i * 5, div_y),
                           (W // 2 + 150 - i * 5, div_y), 1)

        # Login card
        card_w, card_h = 400, 220
        card_x, card_y = W // 2 - card_w // 2, div_y + 40
        # Card shadow
        pygame.draw.rect(self.screen, (10, 10, 15),
                        (card_x + 4, card_y + 4, card_w, card_h), border_radius=12)
        # Card background
        pygame.draw.rect(self.screen, PUBG_DARK,
                        (card_x, card_y, card_w, card_h), border_radius=12)
        pygame.draw.rect(self.screen, PUBG_ACCENT,
                        (card_x, card_y, card_w, card_h), 1, border_radius=12)

        # Card title
        dtxt(self.screen, "■ 战 士 登 录 ■",
             (card_x + card_w // 2, card_y + 30), 22, PUBG_WHITE, True, True, False)

        dtxt(self.screen, "请输入你的代号:",
             (card_x + card_w // 2, card_y + 70), 16, GRAY, ctr=True)
        ...
        pygame.draw.rect(self.screen, (5, 5, 10),
                        (bx + 2, by + 2, bw, bh), border_radius=6)
        # Input background
        input_bg = (40, 40, 55) if self.t % 60 < 45 else (45, 45, 60)
        pygame.draw.rect(self.screen, input_bg,
                        (bx, by, bw, bh), border_radius=6)
        # Input border with pulse
        border_col = PUBG_ACCENT if self.t % 80 < 60 else PUBG_GOLD
        pygame.draw.rect(self.screen, border_col,
                        (bx, by, bw, bh), 2, border_radius=6)

        # Cursor blink
        cursor = "|" if self.t % 40 < 25 else " "
        dtxt(self.screen, self.name_buf + cursor,
             (bx + 12, by + 11), 20, PUBG_WHITE, shd=False)

        # Hint text
        dtxt(self.screen, "按 ENTER 进入战场",
             (card_x + card_w // 2, card_y + 160), 14, GRAY, ctr=True, shd=False)
        dtxt(self.screen, "ESC 退出游戏",
             (card_x + card_w // 2, card_y + 182), 12, DGRAY, ctr=True, shd=False)

        # Bottom decorative line
        pygame.draw.rect(self.screen, PUBG_ACCENT, (0, H - 3, W, 3))

        # Version
        dtxt(self.screen, "v2.0 PUBG Edition", (W - 10, H - 25), 10, DGRAY, shd=False)

    def _draw_menu(self):
        """PUBG Mobile style lobby menu."""
        horizon_y = 420
        # Background layers
        if self.menu_sky:
        ...
        for p in self.cparts:
            a = p["a"]
            c = (int(255 * a * 0.5), int(180 * a * 0.35), int(80 * a * 0.25))
            pygame.draw.circle(self.screen, c, (int(p["x"]), int(p["y"])), int(p["sz"]))

        # Top accent bar
        pygame.draw.rect(self.screen, PUBG_ACCENT, (0, 0, W, 3))

        # Title with PUBG feel
        pulse = math.sin(self.t * 0.03) * 0.12 + 0.88
        title_size = int(50 * pulse)
        dtxt(self.screen, "WARFRONT 3D", (W // 2, 70), title_size, PUBG_ACCENT, True, True)

        # Chicken dinner slogan
        if self.t % 150 < 120:
            dtxt(self.screen, "大吉大利，今晚吃鸡！", (W // 2, 115), 16, CHICKEN_GOLD, ctr=True, shd=False)

        # Player info bar
        info_bar_y = 140
        pygame.draw.rect(self.screen, PUBG_DARK, (W // 2 - 200, info_bar_y, 400, 50), border_radius=8)
        pygame.draw.rect(self.screen, PUBG_ACCENT, (W // 2 - 200, info_bar_y, 400, 50), 1, border_radius=8)

        # Player avatar circle
        av_cx, av_cy = W // 2 - 155, info_bar_y + 25
        pygame.draw.circle(self.screen, PUBG_ACCENT, (av_cx, av_cy), 16)
        dtxt(self.screen, "战", (av_cx, av_cy), 14, BLACK, True, True, False)

        # Player name and stats
        dtxt(self.screen, self.user_name, (av_cx + 25, info_bar_y + 8), 18, PUBG_WHITE, True, shd=False)
        dtxt(self.screen, f"已解锁: {self.max_ul}/9 关 | 金币: {self.coins}",
             (av_cx + 25, info_bar_y + 28), 12, CHICKEN_GOLD, shd=False)

        dtxt(self.screen, "— 选择模式 —", (W // 2, 210), 24, PUBG_WHITE, True, True)
        pygame.draw.line(self.screen, PUBG_ACCENT, (W // 2 - 100, 232), (W // 2 + 100, 232), 1)
        ...
        hvr = bx < pygame.mouse.get_pos()[0] < bx + card_w and by < pygame.mouse.get_pos()[1] < by + card_h
        card_bg = (55, 55, 75) if hvr else PUBG_CARD
        card_bdr = PUBG_GOLD if hvr else PUBG_ACCENT
        # Card shadow
        pygame.draw.rect(self.screen, (10, 10, 18),
                        (bx + 3, by + 3, card_w, card_h), border_radius=12)
        pygame.draw.rect(self.screen, card_bg,
                        (bx, by, card_w, card_h), border_radius=12)
        pygame.draw.rect(self.screen, card_bdr,
                        (bx, by, card_w, card_h), 2, border_radius=12)

        # Card icon (shield/star)
        icon_cx = bx + card_w // 2
        pygame.draw.polygon(self.screen, PUBG_GOLD if hvr else PUBG_ACCENT,
                          [(icon_cx, by + 28), (icon_cx - 20, by + 58),
                           (icon_cx + 20, by + 58)])
        dtxt(self.screen, "★", (icon_cx, by + 45), 18, BLACK, True, True, False)

        dtxt(self.screen, "关卡挑战", (bx + card_w // 2, by + 82), 22, PUBG_WHITE, True, True)
        dtxt(self.screen, "9 个战役关卡", (bx + card_w // 2, by + 108), 14, GRAY, ctr=True)
        dtxt(self.screen, "逐步解锁武器", (bx + card_w // 2, by + 130), 13, PUBG_GREEN, ctr=True)
        dtxt(self.screen, "按 [1] 选择", (bx + card_w // 2, by + 150), 11, DGRAY, ctr=True, shd=False)

        # Classic Mode Card
        bx2 = W // 2 + 20
        hvr2 = bx2 < pygame.mouse.get_pos()[0] < bx2 + card_w and by < pygame.mouse.get_pos()[1] < by + card_h
        card_bg2 = (55, 55, 75) if hvr2 else PUBG_CARD
        card_bdr2 = PUBG_GOLD if hvr2 else PUBG_ACCENT
        pygame.draw.rect(self.screen, (10, 10, 18),
                        (bx2 + 3, by + 3, card_w, card_h), border_radius=12)
        pygame.draw.rect(self.screen, card_bg2,
                        (bx2, by, card_w, card_h), border_radius=12)
        pygame.draw.rect(self.screen, card_bdr2,
                        (bx2, by, card_w, card_h), 2, border_radius=12)

        # Card icon (globe/map)
        icon_cx2 = bx2 + card_w // 2
        pygame.draw.circle(self.screen, PUBG_GOLD if hvr2 else PUBG_BLUE,
                         (icon_cx2, by + 40), 18, 2)
        pygame.draw.line(self.screen, PUBG_GOLD if hvr2 else PUBG_BLUE,
                        (icon_cx2 - 8, by + 34), (icon_cx2 + 12, by + 34), 2)
        pygame.draw.line(self.screen, PUBG_GOLD if hvr2 else PUBG_BLUE,
                        (icon_cx2 - 3, by + 28), (icon_cx2 - 3, by + 52), 2)

        dtxt(self.screen, "经典模式", (bx2 + card_w // 2, by + 82), 22, PUBG_WHITE, True, True)
        dtxt(self.screen, "开放地图探索", (bx2 + card_w // 2, by + 108), 14, GRAY, ctr=True)
        dtxt(self.screen, "100×100 大地图", (bx2 + card_w // 2, by + 130), 13, PUBG_BLUE, ctr=True)
        dtxt(self.screen, "按 [2] 选择", (bx2 + card_w // 2, by + 150), 11, DGRAY, ctr=True, shd=False)
        ...
        # Warehouse button
        wx, wy = bx, btny
        hvr_w = wx < pygame.mouse.get_pos()[0] < wx + small_card_w and wy < pygame.mouse.get_pos()[1] < wy + small_card_h
        w_bg = (55, 55, 75) if hvr_w else PUBG_DARK
        w_bdr = PUBG_GOLD if hvr_w else GRAY
        pygame.draw.rect(self.screen, w_bg, (wx, wy, small_card_w, small_card_h), border_radius=8)
        pygame.draw.rect(self.screen, w_bdr, (wx, wy, small_card_w, small_card_h), 1, border_radius=8)
        dtxt(self.screen, "📦 我的仓库", (wx + small_card_w // 2, wy + 20), 18, PUBG_WHITE, True, True)
        dtxt(self.screen, f"武器 {len(self.warehouse)}件", (wx + small_card_w // 2, wy + 40), 11, GRAY, ctr=True)

        # Shop button
        sx, sy = bx2, btny
        hvr_s = sx < pygame.mouse.get_pos()[0] < sx + small_card_w and sy < pygame.mouse.get_pos()[1] < sy + small_card_h
        s_bg = (55, 55, 75) if hvr_s else PUBG_DARK
        s_bdr = PUBG_GOLD if hvr_s else GRAY
        pygame.draw.rect(self.screen, s_bg, (sx, sy, small_card_w, small_card_h), border_radius=8)
        pygame.draw.rect(self.screen, s_bdr, (sx, sy, small_card_w, small_card_h), 1, border_radius=8)
        dtxt(self.screen, "💰 商店", (sx + small_card_w // 2, sy + 20), 18, PUBG_WHITE, True, True)
        dtxt(self.screen, f"金币: {self.coins}", (sx + small_card_w // 2, sy + 40), 11, CHICKEN_GOLD, ctr=True, shd=False)

        # Keyboard shortcuts
        dtxt(self.screen, "按 [1] 关卡挑战 | 按 [2] 经典模式 | ESC 退出",
             (W // 2, H - 55), 14, GRAY, ctr=True)

        # Exit login button
        ex_x, ex_y = W - 130, H - 80
        mx, my = pygame.mouse.get_pos()
        hvr_ex = ex_x < mx < ex_x + 120 and ex_y < my < ex_y + 35
        pygame.draw.rect(self.screen, (60, 40, 40) if hvr_ex else (35, 20, 20),
                        (ex_x, ex_y, 120, 35), border_radius=6)
        pygame.draw.rect(self.screen, PUBG_RED if hvr_ex else GRAY,
                        (ex_x, ex_y, 120, 35), 2, border_radius=6)
        dtxt(self.screen, "退出登录", (ex_x + 60, ex_y + 17), 16, PUBG_WHITE, True, True)

        # Bottom accent bar
        pygame.draw.rect(self.screen, PUBG_ACCENT, (0, H - 3, W, 3))

        # Bottom bar with triangle decorations
        for i in range(25):
            y = H - 25 + i
            r = i / 25
            c = (int(PUBG_ACCENT[0] * (1 - r * 0.6)),
    ...
    def _draw_select(self):
        """PUBG Mobile style level select screen."""
        # Dark gradient background
        for y in range(H):
            r = y / H
            c = (int(PUBG_BG[0] * (1 - r * 0.3)),
        ...
        # Top bar
        pygame.draw.rect(self.screen, PUBG_ACCENT, (0, 0, W, 3))
        dtxt(self.screen, "选 择 关 卡", (W // 2, 50), 36, PUBG_WHITE, True, True)
        pygame.draw.line(self.screen, PUBG_ACCENT, (W // 2 - 180, 82), (W // 2 + 180, 82), 2)
        dtxt(self.screen, f"金币: {self.coins}", (W - 100, 55), 14, CHICKEN_GOLD, ctr=True, shd=False)
        ...
        for i in range(9):
            ...
            if ul:
                bg = (50, 50, 68) if hvr else PUBG_CARD
                bdr = PUBG_GOLD if hvr else PUBG_ACCENT
            else:
                bg = (20, 20, 26)
                bdr = (40, 40, 48)
            ...
            pygame.draw.rect(self.screen, (8, 8, 14), (rx + 2, ry + 2, rw, rh), border_radius=10)
            pygame.draw.rect(self.screen, bg, (rx, ry, rw, rh), border_radius=10)
            pygame.draw.rect(self.screen, bdr, (rx, ry, rw, rh), 2 if hvr else 1, border_radius=10)

            if ul:
                # Level number badge
                badge_col = PUBG_GOLD if hvr else PUBG_ACCENT
                pygame.draw.circle(self.screen, badge_col, (rx + 22, ry + 22), 15)
                dtxt(self.screen, str(i + 1), (rx + 22, ry + 22), 16, BLACK, True, True, False)
                dtxt(self.screen, LEVELS[i]["name"], (rx + 48, ry + 14), 20, PUBG_WHITE, True)

                # Difficulty stars
                diff = min(5, i // 2 + 1)
                dtxt(self.screen, "★" * diff + "☆" * (5 - diff),
                     (rx + 12, ry + 45), 14, CHICKEN_GOLD, shd=False)
                dtxt(self.screen, f"{LEVELS[i]['waves']} 波敌人", (rx + 12, ry + 66), 12, GRAY, shd=False)
                # Coin hint
                coin_reward = min(5, i // 2 + 1) * 50
                dtxt(self.screen, f"💰 ~{coin_reward}+", (rx + 12, ry + 84), 11, CHICKEN_GOLD, shd=False)
                if LEVELS[i].get("ul"):
                    wn = ", ".join(WEAPONS[w]["name"] for w in LEVELS[i]["ul"])
                    dtxt(self.screen, f"🔓 {wn}", (rx + 12, ry + 98), 10, PUBG_GREEN, shd=False)
            else:
                # Locked icon
                pygame.draw.circle(self.screen, (50, 50, 55), (rx + rw // 2, ry + rh // 2 - 10), 18)
                dtxt(self.screen, "🔒", (rx + rw // 2, ry + rh // 2 - 10), 18, GRAY, ctr=True, shd=False)
                dtxt(self.screen, "LOCKED", (rx + rw // 2, ry + rh // 2 + 16), 13, (55, 55, 60), True, True, False)
        ...
        for x in range(-30, W + 30, 50):
            pygame.draw.polygon(self.screen, (5, 5, 10),
                              [(x, H - 30), (x + 25, H - 30), (x + 40, H), (x + 15, H)])
        dtxt(self.screen, "ESC 返回菜单 | 点击关卡开始战斗",
             (W // 2, H - 18), 14, BLACK, True, True, False)
    ...
    def _draw_hud(self):
        """PUBG Mobile style HUD."""
        # ── Top-left: Player info panel ──
        panel_w, panel_h = 160, 55
        panel_x, panel_y = 8, 8
        ...
        dtxt(self.screen, self.user_name, (panel_x + 10, panel_y + 8), 14, PUBG_WHITE, True, shd=False)
        # HP bar
        hp_ratio = self.hp / self.max_hp
        hp_color = PUBG_GREEN if hp_ratio > 0.5 else PUBG_GOLD if hp_ratio > 0.25 else PUBG_RED
        pygame.draw.rect(self.screen, VGRAY, (panel_x + 10, panel_y + 25, panel_w - 20, 10), border_radius=3)
        pygame.draw.rect(self.screen, hp_color,
                        (panel_x + 10, panel_y + 25, int((panel_w - 20) * hp_ratio), 10), border_radius=3)
        dtxt(self.screen, f"HP {int(self.hp)}", (panel_x + 10, panel_y + 38), 10, PUBG_WHITE, shd=False)

        # ── Top-right: Kill/Score panel ──
        info_w, info_h = 150, 55
        info_x = W - info_w - 8
        pygame.draw.rect(self.screen, (0, 0, 0, 160),
                        (info_x, panel_y, info_w, info_h), border_radius=6)
        pygame.draw.rect(self.screen, PUBG_ACCENT,
                        (info_x, panel_y, info_w, info_h), 1, border_radius=6)
        alive = sum(1 for e in self.enemies if e["alive"])
        dtxt(self.screen, f"击杀 {self.kills}  得分 {self.score}",
             (info_x + info_w // 2, panel_y + 12), 13, PUBG_WHITE, ctr=True, shd=False)
        dtxt(self.screen, f"金币 {self.coins}",
             (info_x + info_w // 2, panel_y + 28), 14, CHICKEN_GOLD, ctr=True, shd=False)
        dtxt(self.screen, f"Wave {min(self.cwave+1,self.twaves)}/{self.twaves}  敌 {alive}",
             (info_x + info_w // 2, panel_y + 44), 11, GRAY, ctr=True, shd=False)

        # ── Kill feed (top center) ──
        kf_y = panel_y + 10
        for kf_text, kf_time, kf_color in self.kill_feed[-4:]:
            alpha = min(1, kf_time / 20)
            kf_c = tuple(int(c * alpha) for c in kf_color)
            dtxt(self.screen, kf_text, (W // 2, kf_y), 14, kf_c, ctr=True, shd=False)
            kf_y += 18
        ...
        for i in range(hh):
            r = i / hh
            c = (int(PUBG_ACCENT[0] * r * 0.15),
                 int(PUBG_ACCENT[1] * r * 0.12),
                 int(PUBG_ACCENT[2] * r * 0.08))
            pygame.draw.line(self.screen, (max(0, c[0]), max(0, c[1]), max(0, c[2])),
                           (0, hy + i), (W, hy + i))
        pygame.draw.line(self.screen, PUBG_ACCENT, (0, hy), (W, hy), 2)

        # ── Weapon info ──
        wk = self.weapons[self.cur_w]
        w = WEAPONS[wk]
        wc = WCOL[wk]

        # Current weapon name
        if self.rld > 0:
            reload_pct = self.rld / w["reload"]
            reload_w = int(120 * (1 - reload_pct))
            pygame.draw.rect(self.screen, DGRAY, (W // 2 - 60, hy + 5, 120, 6), border_radius=3)
            pygame.draw.rect(self.screen, PUBG_GOLD, (W // 2 - 60, hy + 5, reload_w, 6), border_radius=3)
            dtxt(self.screen, "换弹中...", (W // 2 - 60, hy + 14), 16, PUBG_GOLD, True, shd=False)
        else:
            dtxt(self.screen, w["name"], (W // 2 - 60, hy + 10), 17, wc, True, shd=False)
            ammo_text = f"{self.ammo[wk]} / {self.reserve[wk]}"
            dtxt(self.screen, ammo_text, (W // 2 - 60, hy + 30), 20, PUBG_WHITE, True, shd=False)

        # Infinite ammo icon
        ammo_color = YELLOW if self.ammo[wk] > w["mag"] * 0.3 else PUBG_RED
        dtxt(self.screen, "∞" if self.reserve[wk] > 0 else "×",
             (W // 2 + 60, hy + 20), 22, ammo_color, ctr=True, shd=False)

        # ── Weapon slots ──
        for i in range(len(self.weapons)):
            sx2 = W // 2 - 60 + i * 45
            slot_color = PUBG_ACCENT if i == self.cur_w else DGRAY
            slot_bg = (40, 40, 55) if i == self.cur_w else (20, 20, 30)
            pygame.draw.rect(self.screen, slot_bg, (sx2, hy + 48, 40, 14), border_radius=3)
            pygame.draw.rect(self.screen, slot_color, (sx2, hy + 48, 40, 14), 1, border_radius=3)
            slot_wi = self.weapons[i]
            dtxt(self.screen, WEAPONS[slot_wi]["name"][:2],
                 (sx2 + 2, hy + 48), 9, slot_color, shd=False)
            dtxt(self.screen, f"[{i + 1}]", (sx2 + 22, hy + 48), 8, GRAY, shd=False)

        # ── Grenades (right side) ──
        gx_base = W - 135
        dtxt(self.screen, "手雷", (gx_base + 30, hy + 8), 12, PUBG_WHITE, ctr=True, shd=False)
        for i in range(5):
            gx = gx_base + i * 20
            has_nade = i < self.grenades
            nade_col = (70, 150, 70) if has_nade else DGRAY
            pygame.draw.ellipse(self.screen, nade_col, (gx, hy + 22, 14, 18))
            if has_nade:
                pygame.draw.line(self.screen, PUBG_GOLD, (gx + 7, hy + 22), (gx + 7, hy + 18), 2)

        # ── Mode indicator ──
        mode_txt = "战役" if self.mode == "level" else "经典"
        mode_col = PUBG_GOLD if self.mode == "level" else PUBG_BLUE
        dtxt(self.screen, f"[{mode_txt}] V视角 M地图", (8, H - hh - 18), 11, mode_col, shd=False)

        # Level name at top
        if self.mode == "level" and self.clvl >= 0:
            dtxt(self.screen, LEVELS[self.clvl]["name"],
                 (panel_x + panel_w + 10, panel_y + 8), 14, PUBG_WHITE, True, shd=False)
    ...
    def _draw_overlay(self, won):
        """PUBG Mobile style mission complete / failed overlay with coin rewards."""
        # Dark overlay
        ...
        # Decorative border
        border_rect = pygame.Rect(W // 2 - 280, H // 2 - 180, 560, 360)
        pygame.draw.rect(self.screen, PUBG_DARK, border_rect, border_radius=16)
        pygame.draw.rect(self.screen, PUBG_ACCENT if won else PUBG_RED, border_rect, 3, border_radius=16)

        # Top accent line
        pygame.draw.rect(self.screen, PUBG_ACCENT if won else PUBG_RED,
                        (W // 2 - 200, H // 2 - 175, 400, 3), border_radius=2)

        if won:
            # ──── VICTORY SCREEN ────
            # Chicken dinner reference!
            if self.t % 120 < 80:
                chicken_pulse = math.sin(self.t * 0.08) * 0.3 + 0.7
                c_col = tuple(int(CHICKEN_GOLD[i] * chicken_pulse) for i in range(3))
                dtxt(self.screen, "🍗 大吉大利，今晚吃鸡！ 🍗",
                     (W // 2, H // 2 - 140), 28, c_col, ctr=True)

            dtxt(self.screen, "MISSION COMPLETE", (W // 2, H // 2 - 95), 42, PUBG_GOLD, True, True)
            dtxt(self.screen, "任 务 完 成", (W // 2, H // 2 - 55), 24, PUBG_GREEN, ctr=True)

            # ── Divider ──
            div_y = H // 2 - 30
            pygame.draw.line(self.screen, PUBG_ACCENT, (W // 2 - 200, div_y), (W // 2 + 200, div_y), 1)

            # ── Stats Row ──
            stat_y = H // 2 - 10
            dtxt(self.screen, f"击杀", (W // 2 - 150, stat_y), 14, GRAY, ctr=True)
            dtxt(self.screen, str(self.kills), (W // 2 - 150, stat_y + 20), 22, PUBG_WHITE, ctr=True)
            dtxt(self.screen, f"得分", (W // 2, stat_y), 14, GRAY, ctr=True)
            dtxt(self.screen, str(self.score), (W // 2, stat_y + 20), 22, PUBG_WHITE, ctr=True)
            dtxt(self.screen, f"时间", (W // 2 + 150, stat_y), 14, GRAY, ctr=True)
            dtxt(self.screen, f"{int(self.playtime)}s", (W // 2 + 150, stat_y + 20), 18, PUBG_WHITE, ctr=True)

            # ── COIN REWARD SECTION (prominent!) ──
            coin_y = H // 2 + 45
            # Coin glow effect
            coin_pulse = math.sin(self.t * 0.06) * 0.2 + 0.8
            glow_r = int(80 * coin_pulse)
            for layer in range(3):
                alpha = 40 - layer * 12
                glow_c = (CHICKEN_GOLD[0], CHICKEN_GOLD[1], CHICKEN_GOLD[2])
                r = glow_r + layer * 10
                pygame.draw.circle(self.screen, glow_c + (alpha,),
                                 (W // 2, coin_y + 30), r, 2)

            # Big coin icon
            pygame.draw.circle(self.screen, CHICKEN_GOLD, (W // 2, coin_y + 30), 28)
            pygame.draw.circle(self.screen, CHICKEN_YELLOW, (W // 2, coin_y + 30), 22, 2)
            dtxt(self.screen, "💰", (W // 2, coin_y + 30), 28, BLACK, ctr=True, shd=False)

            # Coin amount with animation
            coin_display = self.coin_earned
            if self.coin_anim > 0:
                coin_display = int(self.coin_earned * (1 - self.coin_anim / 120))

            dtxt(self.screen, f"+{coin_display}", (W // 2, coin_y + 70), 36, CHICKEN_GOLD, ctr=True)
            dtxt(self.screen, "金币奖励", (W // 2, coin_y + 95), 14, PUBG_ACCENT, ctr=True)

            # Total coins
            dtxt(self.screen, f"持有金币: {self.coins}",
                 (W // 2, coin_y + 120), 15, CHICKEN_GOLD, ctr=True, shd=False)
            ...
            if self.mode == "level" and LEVELS[self.clvl].get("ul"):
                ...
                unlock_y = coin_y + 145
                pygame.draw.rect(self.screen, (20, 50, 20),
                                (W // 2 - 150, unlock_y - 10, 300, 28), border_radius=5)
                pygame.draw.rect(self.screen, PUBG_GREEN,
                                (W // 2 - 150, unlock_y - 10, 300, 28), 1, border_radius=5)
                dtxt(self.screen, f"🔓 解锁武器: {wn}",
                     (W // 2, unlock_y + 4), 15, PUBG_GREEN, ctr=True)
        else:
            # ──── DEFEAT SCREEN ────
            dtxt(self.screen, "MISSION FAILED", (W // 2, H // 2 - 110), 42, PUBG_RED, True, True)
            dtxt(self.screen, "任 务 失 败", (W // 2, H // 2 - 70), 24, (200, 100, 100), ctr=True)

            # Divider
            div_y = H // 2 - 40
            pygame.draw.line(self.screen, PUBG_RED, (W // 2 - 200, div_y), (W // 2 + 200, div_y), 1)

            # Stats
            stat_y = H // 2 - 15
            dtxt(self.screen, f"击杀: {self.kills}", (W // 2 - 100, stat_y), 16, PUBG_WHITE, ctr=True)
            dtxt(self.screen, f"得分: {self.score}", (W // 2 + 100, stat_y), 16, PUBG_WHITE, ctr=True)

            # Encourage message
            dtxt(self.screen, "不要太灰心，重新来过！", (W // 2, stat_y + 40), 16, GRAY, ctr=True)
            dtxt(self.screen, f"当前金币: {self.coins}", (W // 2, stat_y + 65), 14, CHICKEN_GOLD, ctr=True, shd=False)

        # ── Bottom: Continue hint ──
        if self.t % 80 < 55:
            dtxt(self.screen, "按 ENTER 继续 | ESC 返回菜单",
                 (W // 2, H // 2 + 165), 16, PUBG_ACCENT, ctr=True)
